...
class FastGameTheoryClusterer:
    ...
    def smart_threshold_selection(self, shapley_matrix):
        """Aggressive threshold selection to force fewer, better clusters."""
        upper_triangle = shapley_matrix[np.triu_indices_from(shapley_matrix, k=1)]
        non_zero_values = upper_triangle[upper_triangle > 0]
        
        if len(non_zero_values) == 0:
            return np.percentile(upper_triangle, 95)
        
        # Force very high threshold to get 3-5 clusters maximum
        # This will create fewer, larger, more cohesive clusters
        best_threshold = np.percentile(non_zero_values, 98)  # Very high threshold
        
        # Test threshold to ensure we get reasonable cluster count
        test_labels = self._form_coalitions(shapley_matrix, best_threshold)
        n_clusters = len(np.unique(test_labels))
        
        # If still too many clusters, increase threshold further
        if n_clusters > 8:
            best_threshold = np.percentile(non_zero_values, 99)
            test_labels = self._form_coalitions(shapley_matrix, best_threshold)
            n_clusters = len(np.unique(test_labels))
        
        # Final fallback - use maximum threshold if still too many
        if n_clusters > 10:
            best_threshold = np.max(non_zero_values) * 0.95
        
        print(f"🎯 High threshold for fewer clusters: {best_threshold:.4f}")
    ...
    def _form_coalitions(self, shapley_matrix, threshold):
        ...
        for point in list(unassigned):
            ...
            if best_cluster != -1:
                ...
                if cluster_id < 5:  # Strict limit - max 5 clusters
...
def generate_demo_data(n_samples=120):
    """Generate overlapping clusters perfect for Game Theory coalition formation."""
    np.random.seed(42)
    
    print(f"📊 Generating {n_samples} coalition-friendly records...")
    
    # Create overlapping business groups that benefit from coalition analysis
    ...
    # Group 1: High-value Steel (overlaps with Premium vendors)
    for i in range(n_per_group):
        data['material'].append('Steel')
        data['vendor_type'].append(np.random.choice(['Premium', 'Standard'], p=[0.7, 0.3]))
        data['quantity'].append(int(np.random.normal(100, 20)))
        data['price_per_unit'].append(np.random.normal(18, 3))
        data['country'].append(np.random.choice(['Germany', 'USA', 'Japan'], p=[0.5, 0.3, 0.2]))
        data['payment_terms'].append(np.random.choice([30, 45], p=[0.6, 0.4]))
    
    # Group 2: Medium-value Mixed (bridge group - creates coalition opportunities)
    for i in range(n_per_group):
        data['material'].append(np.random.choice(['Steel', 'Aluminum'], p=[0.4, 0.6]))
        data['vendor_type'].append(np.random.choice(['Standard', 'Premium'], p=[0.8, 0.2]))
        data['quantity'].append(int(np.random.normal(60, 15)))
        data['price_per_unit'].append(np.random.normal(12, 2))
        data['country'].append(np.random.choice(['USA', 'Canada', 'Germany'], p=[0.4, 0.4, 0.2]))
        data['payment_terms'].append(np.random.choice([45, 60], p=[0.7, 0.3]))
    
    # Group 3: Standard Aluminum (coalition-friendly with Group 2)
    for i in range(n_per_group):
        data['material'].append('Aluminum')
        data['vendor_type'].append(np.random.choice(['Standard', 'Discount'], p=[0.6, 0.4]))
        data['quantity'].append(int(np.random.normal(50, 12)))
        data['price_per_unit'].append(np.random.normal(8, 1.5))
        data['country'].append(np.random.choice(['USA', 'China'], p=[0.6, 0.4]))
        data['payment_terms'].append(np.random.choice([60, 90], p=[0.8, 0.2]))
    
    # Group 4: Mixed Low-cost (overlapping characteristics)
    for i in range(n_samples - 3 * n_per_group):
        data['material'].append(np.random.choice(['Plastic', 'Aluminum'], p=[0.7, 0.3]))
        data['vendor_type'].append(np.random.choice(['Discount', 'Standard'], p=[0.7, 0.3]))
        data['quantity'].append(int(np.random.normal(35, 10)))
        data['price_per_unit'].append(np.random.normal(5, 1))
        data['country'].append(np.random.choice(['China', 'India'], p=[0.6, 0.4]))
        data['payment_terms'].append(np.random.choice([60, 90], p=[0.5, 0.5]))
    
    # Ensure positive values and calculate total
    for i in range(len(data['quantity'])):
        data['quantity'][i] = max(1, data['quantity'][i])
        data['price_per_unit'][i] = max(0.5, data['price_per_unit'][i])
    
    data['total_amount'] = [q * p for q, p in zip(data['quantity'], data['price_per_unit'])]
    
    df = pd.DataFrame(data)
    print(f"✅ Coalition-friendly dataset ready: {df.shape}")
    print("   - Overlapping groups favor coalition formation")
    print("   - Multiple similarity dimensions for Shapley analysis")
    return df
