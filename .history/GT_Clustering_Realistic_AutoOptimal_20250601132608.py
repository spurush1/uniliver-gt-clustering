...
class RealisticGameTheoryClusterer:
    ...
    def form_coalitions_with_threshold(self, shapley_matrix, threshold):
        ...
        print(f"   Found {len(connections)} strong connections above threshold {threshold:.4f}")
        ...
        for shapley_val, i, j in connections:
            if i in unassigned and j in unassigned:
                ...
                if avg_stability > threshold * 0.6:  # Stricter stability requirement
        ...
        print(f"   Assigning {len(remaining_points)} remaining points intelligently...")
        ...
        for point in remaining_points:
            if cluster_id == 0:
                ...
                min_utility_threshold = threshold * 0.4  # More lenient for remaining points
        ...
        avg_cluster_size = n / final_clusters
        print(f"   ✅ Formed {final_clusters} coalitions, avg size: {avg_cluster_size:.1f}")
    ...
    def select_optimal_clustering(self, thresholds, cluster_counts, stability_scores):
        ...
        for i, (threshold, n_clusters, stability) in enumerate(zip(thresholds, cluster_counts, stability_scores)):
            ...
            if cluster_ratio <= 1.0:
                cluster_size_score = cluster_ratio  # Reward up to optimal
            else:
                cluster_size_score = 1.0 / cluster_ratio  # Penalize excess
            ...
            if avg_cluster_size >= 3:  # Prefer clusters with at least 3 members
            ...
            if n_clusters > self.n_samples * 0.3:  # More than 30% are singletons
