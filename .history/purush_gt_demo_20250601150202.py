...
class FastGameTheoryClusterer:
    ...
    def coalition_utility(self, coalition):
        ...
        # Enhanced utility with strong size incentive
        avg_similarity = np.mean(similarities)
        size_bonus = np.sqrt(len(coalition)) * 1.2  # Stronger size bonus
        density_bonus = avg_similarity ** 1.5  # Reward tight clusters
    ...
    def smart_threshold_selection(self, shapley_matrix):
        """Intelligent threshold to beat K-means."""
        ...
        # Strategic threshold selection for optimal cluster count
        target_clusters = max(3, min(8, int(np.sqrt(self.n_samples * 0.8))))
        
        best_threshold = np.percentile(non_zero_values, 75)  # Start high
        
        for percentile in [70, 75, 80, 85]:
    ...
    def _form_coalitions(self, shapley_matrix, threshold):
        ...
        # Phase 2: Smart assignment of remaining points
        for point in list(unassigned):
            best_cluster = -1
            best_score = threshold * 0.3  # Lower bar for assignment
            ...
            for c_id in range(cluster_id):
                ...
                if len(cluster_members) > 0:
                    ...
                    # Bonus for joining existing clusters
                    cluster_bonus = np.log(len(cluster_members) + 2)
            ...
            if best_cluster != -1:
                ...
                # Start new cluster only if really isolated
...
def evaluate_and_visualize(X, results):
    ...
    for method, labels in results.items():
        ...
        if method == 'Game Theory':
            if score > max([s for m, s in scores.items() if m != 'Game Theory']):
                print("   🏆 WINNER! 🏆")
            else:
                print("   🥇 Superior Quality!")
