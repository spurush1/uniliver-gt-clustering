...
class FastGameTheoryClusterer:
    ...
    def smart_threshold_selection(self, shapley_matrix):
        """Optimized threshold for overlapping coalition-friendly data."""
        upper_triangle = shapley_matrix[np.triu_indices_from(shapley_matrix, k=1)]
        non_zero_values = upper_triangle[upper_triangle > 0]
        
        if len(non_zero_values) == 0:
            return np.percentile(upper_triangle, 90)
        
        # For overlapping data, use moderate threshold to find natural coalitions
        # Test multiple thresholds to find optimal balance
        best_threshold = 0.0
        best_score = -1
        
        for percentile in [80, 85, 90, 92, 94]:
            ...
            # Score clusters based on reasonable count (3-6 optimal)
            if 3 <= n_clusters <= 6:
                score = 1.0
            elif 2 <= n_clusters <= 8:
                score = 0.8
        ...
        # Fallback if no good threshold
        if best_threshold == 0.0:
            best_threshold = np.percentile(non_zero_values, 85)
        
        print(f"🎯 Coalition threshold: {best_threshold:.4f}")
